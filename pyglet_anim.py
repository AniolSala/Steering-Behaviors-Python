...
from numpy import array, ones, pi, random, linspace, sin, cos, angle, loadtxt, savetxt
...
class MyWindow(pyglet.window.Window):
    ...
    def behaviors(self, arrow):
        ...
        closeF = arrow.seek(self.foodlist, arrow.dna[2])
        closeP = arrow.seek(self.poisonlist, arrow.dna[3])
            ...
            if self.showdna == -1:
                ...
                for ang in linspace(0, 2 * pi, 35):
                    pyglet.gl.glVertex2f(
                        arrow.dna[2] * cos(ang), arrow.dna[2] * sin(ang))
                ...
                for ang in linspace(0, 2 * pi, 35):
                    pyglet.gl.glVertex2f(
                        arrow.dna[3] * cos(ang), arrow.dna[3] * sin(ang))
    ...
    def update(self, dt):
        ...
        if random.rand() < 0.12 and len(self.foodlist) <= self.maxfood:
            self.foodlist.append(
                Food(random.randint(self.width), random.randint(self.height)))
        if random.rand() < .01 and len(self.poisonlist) <= self.maxpoison:
            self.poisonlist.append(
                Poison(random.randint(self.width), random.randint(self.height)))
